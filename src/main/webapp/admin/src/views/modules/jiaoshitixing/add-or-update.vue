    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="发布内容" prop="fabuneirong">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.fabuneirong" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.fabuneirong" label="发布内容" prop="fabuneirong">
                    <span v-html="ruleForm.fabuneirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.fabushijian = true;
          ...
          if(o=='fabuneirong'){
            this.ruleForm.fabuneirong = obj[o];
	    this.ro.fabuneirong = true;
            continue;
          }
          if(o=='xuehao'){
	...
	this.ruleForm.fabuneirong = this.ruleForm.fabuneirong.replace(reg,'../../../ssmgp6yp/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiaoshitixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiaoshitixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
