<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="选择日期" prop="xuanzeriqi">
            <el-date-picker
        ...
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.xuanzeriqi" label="选择日期" prop="xuanzeriqi">
              <el-input v-model="ruleForm.xuanzeriqi" 
                placeholder="选择日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="作业要求" prop="zuoyeyaoqiu">
          <el-input v-model="ruleForm.zuoyeyaoqiu" 
              placeholder="作业要求" clearable  :readonly="ro.zuoyeyaoqiu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="作业要求" prop="zuoyeyaoqiu">
              <el-input v-model="ruleForm.zuoyeyaoqiu" 
                placeholder="作业要求" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      this.$http({
		...
		if(json.xingming!=''&&json.xingming){
          ...
          this.$message.error(data.msg);
            ...
            this.zhuangtaiOptions = "已完成,未完成".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `xuanzezuoye/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xuanzezuoye/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xuanzezuoyeCrossAddOrUpdateFlag = false;
						 this.parent.search();
